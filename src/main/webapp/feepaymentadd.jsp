...
out.print(result);
return; // STOP PAGE HERE (IMPORTANT)
}
...
ResultSet rs = st.executeQuery("SELECT MAX(PaymentID)+1 FROM feepayments");
...
background: linear-gradient(120deg,#0f0c29,#302b63,#24243e,#ff2e63,#6a0dad);
...
h1{color:#6a0dad;font-size:50px;}
...
button{
background: linear-gradient(135deg,#6a0dad,#ff4ecd);
color:white;
padding:15px 35px;
border:none;
border-radius:12px;
...
if(name === "Not exist"){
document.getElementById("studentName").value = "";
document.getElementById("studentName").placeholder = "Student Not Found";
}else{
...
xhttp.open(
"GET",
"addPayment.jsp?studentID_ajax="+sid+"&t="+new Date().getTime(),
true
);
